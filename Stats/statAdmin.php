...
<select name="course_select" class="course_select">
    ...
    <select name="user" class="course_select">
        <?php
        include '../Connector/PDO_connect.php';
        $stmt = $conn->prepare("SELECT * FROM USERS WHERE id_user != 14");
        $stmt->execute();
        $resultuser = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $_SESSION['usrIDS'] = $resultuser;
        if (!empty($resultuser)) {
            echo "<option id=''hidden>Select Course</option>";
            foreach ($resultuser as $row) {
                echo "<option value='" . $row["id_user"] . "' data-user-name='" . $row["user_name"] . "'>" . $row["first_name"] .$row["last_name"] . "</option>";
            }
        } else {
            echo "<option>No Courses Have Been Added</option>";
        }
